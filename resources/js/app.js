 ...
 */
function saveCoordsAndPermission(latitude, longitude) {
    localStorage.setItem(GEOLOCATION_KEY, 'granted');
    localStorage.setItem(GEOLOCATION_COORDS_KEY, JSON.stringify({ latitude, longitude }));
    ...
    if (browserPermission === 'granted' || metaPermission === 'granted' || isGeolocationPermissionGranted()) {
        ...
        navigator.geolocation.getCurrentPosition(
            (position) => {
                ...
                saveCoordsAndPermission(latitude, longitude);
            ...
            (err) => {
                hideAutoDetectSpinner();
                // If live GPS fails (e.g. timeout), fall back to DB cached coordinates instantly
                if (metaLat && metaLon && metaLat !== '' && metaLon !== '') {
                    fetchWeatherByCoords(parseFloat(metaLat), parseFloat(metaLon), true);
                }
            },
...
window.fetchWeatherByLocation = function () {
    ...
    if (btn) {
        btn.disabled = true;
        btn.innerHTML = `<span class="animate-spin inline-block mr-2">⏳</span> ${document.documentElement.lang === 'hi' ? 'खोज की जा रही है...' : 'Fetching...'}`;
    }
    if (statusEl) statusEl.textContent = document.documentElement.lang === 'hi' ? 'स्थान का पता लगाया जा रहा है...' : 'Detecting location...';

    // High accuracy, timeout set to 10 seconds
    navigator.geolocation.getCurrentPosition(
        (position) => {
            const { latitude, longitude } = position.coords;
            saveCoordsAndPermission(latitude, longitude);
        ...
        (err) => {
            ...
            if (err.code === err.PERMISSION_DENIED) {
                markPermissionDenied();
                showLocationDeniedWarning();
                const msg = document.documentElement.lang === 'hi' ? 'स्थान पहुंच अस्वीकार कर दी गई। कृपया मैन्युअल रूप से दर्ज करें।' : 'Location access denied. Please enter city manually.';
                showToast(msg, 'info');
            } else {
                const msg = document.documentElement.lang === 'hi' ? 'स्थान पहुंच विफल रही। कृपया मैन्युअल रूप से भरें।' : 'Location access failed. Please fill weather manually.';
                showToast(msg, 'info');
            }
        },
        {
        ...
        try {
            ...
            container.innerHTML = suitable.map(c => `
                <span class="inline-flex items-center gap-1 px-3 py-1 bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300 rounded-full text-xs font-semibold">
                    🌱 ${c.translated_name || c.name} <span class="opacity-70">(${c.match_score}%)</span>
                </span>
            `).join('');
        } catch (e) { /* silent fail */ }
    }, 600);
    ...
    cityInput.addEventListener('input', () => {
        ...
        suggestionsList.innerHTML = `
            <li class="px-4 py-3 text-sm text-slate-400 dark:text-slate-500 flex items-center gap-2">
                <span class="animate-spin text-base">⏳</span> ${loadingMsg}
            </li>
        `;
        ...
        cityDebounce = setTimeout(async () => {
            try {
                ...
                const errorMsg = isHi ? 'खोज विफल रही' : 'Search failed';
...
// ─── Core weather fetch by coordinates with deduplication ─────────────────────
let lastWeatherFetchTime = 0;
const WEATHER_FETCH_MIN_INTERVAL = 5000; // Minimum 5 seconds between fetches
...
window.fetchWeatherByCoords = async function (latitude, longitude, isAuto = false) {
    ...
    if (now - lastWeatherFetchTime < WEATHER_FETCH_MIN_INTERVAL) {
        ...
        if (btn && !isAuto) {
            const isHi = document.documentElement.lang?.startsWith('hi');
            btn.disabled = false;
            btn.innerHTML = isHi ? `📡 मेरे स्थान का मौसम प्राप्त करें` : `📡 Fetch My Location Weather`;
        }
    ...
    if (btn && !isAuto) {
        btn.disabled = true;
        btn.innerHTML = `<span class="animate-spin inline-block mr-2">⏳</span> Fetching...`;
    }
    ...
    try {
        ...
        if (data.success) {
            ...
            if (data.annual_rain !== undefined && data.annual_rain !== null) {
                setFieldValue('rainfall', String(Math.round(data.annual_rain * 10) / 10));
            } else {
            ...
            if (statusEl) {
                ...
                const formatForecastHour = (time) => {
                    const clockTime = String(time).split('T')[1]?.slice(0, 5);
                    if (!clockTime) return time;
                ...
                const upcomingHours = (data.hourly ?? []).filter(h => h.temp !== undefined && h.temp !== null).slice(0, 5);
                ...
                // Build a compact status card with integer values for clarity
                statusEl.innerHTML = `
                    <div class="mt-3 p-4 bg-green-50/80 dark:bg-green-900/20 rounded-xl border border-green-200 dark:border-green-800/50 shadow-sm transition-all">
                        <div class="flex justify-between items-start mb-3">
                            <div>
                                <p class="font-bold text-green-900 dark:text-green-100 text-base">📍 ${data.city || defaultLoc}</p>
                                <p class="text-xs text-green-600 dark:text-green-400 mt-0.5 capitalize">${translatedCondition}</p>
                            </div>
                        ...
                        <div class="grid grid-cols-3 gap-3 bg-white dark:bg-gray-800 rounded-lg p-3 border border-green-100 dark:border-gray-700 shadow-sm text-center">
                            <div class="flex flex-col items-center">
                                <span class="text-gray-400 text-[10px] uppercase tracking-wider mb-1">${isHi ? 'नमी' : 'Humidity'}</span>
                                <span class="font-semibold text-gray-800 dark:text-gray-200 text-sm">${humidityText}</span>
                            </div>
                            <div class="flex flex-col items-center">
                                <span class="text-gray-400 text-[10px] uppercase tracking-wider mb-1">${isHi ? 'हवा' : 'Wind'}</span>
                                <span class="font-semibold text-gray-800 dark:text-gray-200 text-sm">${windText}</span>
                            </div>
                            <div class="flex flex-col items-center">
            ...
            const errorMsg = document.documentElement.lang?.startsWith('hi') ? 'मौसम लोड नहीं किया जा सका।' : 'Could not load weather.';
            showToast(data.message || errorMsg, 'error');
        }
    } catch {
        const netErrorMsg = document.documentElement.lang?.startsWith('hi') ? 'मौसम प्राप्त करने में नेटवर्क त्रुटि।' : 'Network error fetching weather.';
