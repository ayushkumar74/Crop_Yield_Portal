        ...
        <div class="hidden sm:flex-1 sm:flex sm:gap-2 sm:items-center sm:justify-between">
                ...
                <p class="text-sm text-gray-700 leading-5 dark:text-gray-300">
                    @if(app()->getLocale() === 'hi')
                        @if ($paginator->firstItem())
                            कुल <span class="font-medium text-slate-900 dark:text-white">{{ $paginator->total() }}</span> {{ __('messages.pagin_results') }} में से <span class="font-medium text-slate-900 dark:text-white">{{ $paginator->firstItem() }}</span> से <span class="font-medium text-slate-900 dark:text-white">{{ $paginator->lastItem() }}</span> {{ __('messages.pagin_showing') }}
                        @else
                            <span class="font-medium text-slate-900 dark:text-white">{{ $paginator->count() }}</span> {{ __('messages.pagin_results') }} {{ __('messages.pagin_showing') }}
                        @endif
                    @else
                        {{ __('messages.pagin_showing') }}
                        @if ($paginator->firstItem())
                            <span class="font-medium text-slate-900 dark:text-white">{{ $paginator->firstItem() }}</span>
                            {{ __('messages.pagin_to') }}
                            <span class="font-medium text-slate-900 dark:text-white">{{ $paginator->lastItem() }}</span>
                        @else
                            {{ $paginator->count() }}
                        @endif
                        {{ __('messages.pagin_of') }}
                        <span class="font-medium text-slate-900 dark:text-white">{{ $paginator->total() }}</span>
                        {{ __('messages.pagin_results') }}
                    @endif
                </p>
            </div>
            ...
            <div>
                <span class="inline-flex rtl:flex-row-reverse shadow-sm rounded-md">
                    ...
                    {{-- Previous Page Link --}}
                    @if ($paginator->onFirstPage())
                        <span aria-disabled="true" aria-label="{{ __('messages.pagin_previous') }}">
                    ...
                    {{-- Next Page Link --}}
                    @if ($paginator->hasMorePages())
                        <a href="{{ $paginator->nextPageUrl() }}" rel="next" class="inline-flex items-center px-2 py-2 -ml-px text-sm font-medium text-gray-500 bg-white border border-gray-300 rounded-r-md leading-5 hover:text-gray-400 focus:outline-none focus:ring ring-gray-300 focus:border-blue-300 active:bg-gray-100 active:text-gray-500 transition ease-in-out duration-150 dark:bg-gray-800 dark:border-gray-600 dark:active:bg-gray-700 dark:focus:border-blue-800 dark:text-gray-300 dark:hover:bg-gray-900 dark:hover:text-gray-300" aria-label="{{ __('messages.pagin_next') }}">
                            <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd" />
                            </svg>
                        </a>
                    @else
                        <span aria-disabled="true" aria-label="{{ __('messages.pagin_next') }}">
